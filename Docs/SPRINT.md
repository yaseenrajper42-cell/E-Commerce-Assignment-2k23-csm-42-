...
- **Users â†’ Orders:** One user can place many orders, while each order belongs to one user (**1:N**).
- **Users â†’ Carts:** A user can have zero or one active cart in this design, and each cart belongs to one user (**1:0..1**).
- **Carts â†’ Cart_Items:** One cart can contain multiple cart items, while each cart item belongs to one cart (**1:N**).
- **Products â†’ Cart_Items:** A product can appear in many users' carts, while each cart item refers to one product (**1:N**).
- **Orders â†’ Order_Items:** One order contains one or more order items, and each order item belongs to one order (**1:N**).
- **Products â†’ Order_Items:** A product can occur in many order items, while each order item refers to one product (**1:N**).
- **Categories â†’ Products:** One category can contain multiple products, while each product belongs to one category (**1:N**).

This structure keeps the main entities separate and avoids storing repeated product or order information in a single table. It should also be enough for the basic shopping workflow required for the MVP.